...
def load_and_preprocess(file_name="https://storage.googleapis.com/telco-churn-123/telco-churn.csv"):
    ...
    if len(y) < 10 or min_class_count < 2:
        ...
        np.savetxt("data/X_train.csv", X_train, delimiter=",", fmt="%.8f")
        np.savetxt("data/X_test.csv", X_test, delimiter=",", fmt="%.8f")
        np.savetxt("data/y_train.csv", y_train, delimiter=",", fmt="%.8f")
        np.savetxt("data/y_test.csv", y_test, delimiter=",", fmt="%.8f")
        print("Data saved to data/X_train.csv, data/X_test.csv, data/y_train.csv, data/y_test.csv")
